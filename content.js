...
(function () {
    ...
    if (!document.getElementById(STYLE_ID)) {
        ...
        style.textContent = `
            ...
            .wh-input {
                ...
                font-size: 13px; color: var(--wh-text);
                background: white; font-family: var(--wh-font);
  ...
  <a style="display: inline-block; background: linear-gradient(135deg, #667eea, #764ba2); color: white; padding: 14px 36px; border-radius: 50px; font-size: 17px; font-weight: bold; text-decoration: none;">🚀 立即行动</a>
    ...
    async function importWechatArticle(url) {
        ...
        const html = extractWechatArticleHTML(htmlText);
        if (!html) throw new Error('未解析到正文内容');
        return html;
    }
    ...
    function showDialog() {
        ...
        <div id="wh-dialog">
            ...
            <div class="wh-editor-area">
                <div class="wh-import-area">
                    <div class="wh-import-title">公众号链接导入</div>
                    <div class="wh-import-row">
                        <input class="wh-input" id="wh-url" placeholder="粘贴 https://mp.weixin.qq.com/s/... 文章链接" />
                        <button class="wh-btn wh-btn-import" id="wh-import">导入</button>
                    </div>
                    <div class="wh-import-hint">抓取正文并直接写入编辑器（与下方 HTML 插入功能独立）</div>
                </div>
        ...
        // 导入公众号链接
        document.getElementById('wh-import').onclick = async () => {
            const url = (urlInput.value || '').trim();
            if (!url) { toast('请粘贴公众号文章链接', 'warning'); return; }
            if (!/https?:\/\/mp\.weixin\.qq\.com\//i.test(url)) {
                toast('仅支持 mp.weixin.qq.com 链接', 'warning'); return;
            }
            try {
                toast('正在拉取文章内容...', 'warning');
                const html = await importWechatArticle(url);
                overlay.remove();
                setTimeout(() => smartInsertFallback(html), 150);
            } catch (e) {
                log.warn('导入失败:', e.message);
